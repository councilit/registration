...
import { deleteFile,  fetchFilesFromSelamMinster } from "../../../store/features/file.slice";
import { fileUrl } from "../../../utils/file-url.util";
import DeleteForeverIcon from "@mui/icons-material/DeleteForever";
import OpenInNewIcon from '@mui/icons-material/OpenInNew';
import AddMemberFilesFromSM from "./AddMemberFilesFromSM";
...
const MemberFilesFromSelamMinster: React.FC<MemberFilesProps> = ({ memberId }) => {
  ...
  const { filesFromSelamMinster:files, status, task } = useAppSelector((state) => state.file);
  const dispatch = useAppDispatch();
  useEffect(() => {
    if (memberId) {
      dispatch(fetchFilesFromSelamMinster({ member: memberId,  }));
    }
  }, [memberId, dispatch]);
  ...
  const loading = status === "loading" && task === "fetch-files-from-selam-minster";
           ...
           {files.length > 0 &&
              files.map((file) => (
                ...
                handleDownload(fileUrl("file", file.file!))
                          ...
                          <Button variant="outlined" startIcon={<OpenInNewIcon />} sx={{ ml: 1, color: '#2563eb', borderColor: '#2563eb33', borderRadius: 6, textTransform: 'none' }} onClick={() => handleView(fileUrl('file', file.file!), file.fileName)}>View</Button>
                        </Stack>
                      </Stack>
                    </Grid>
...
export default MemberFilesFromSelamMinster;
