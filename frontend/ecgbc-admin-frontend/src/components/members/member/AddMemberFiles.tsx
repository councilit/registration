...
const AddMemberFiles: React.FC<AddMemberFilesProps> = ({ memberId, handleModalClose }) => {
  ...
  const onInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // reset value to allow re-selecting same file name again
        ...
        <FormControl fullWidth sx={{ my: 1 }}>
         ...
         >
            ...
            <Typography variant="caption" color="text.secondary" align="center">
              Max {MAX_FILES} files • {MAX_TOTAL_SIZE_MB}MB total • Types: {ACCEPTED_TYPES}
            </Typography>
            <input
              ref={inputRef}
              type="file"
          ...
          <Stack direction="row" flexWrap="wrap" useFlexGap gap={1} mt={1}>
            {files.map((f, idx) => (
              <Chip key={`${f.name}-${f.size}-${f.lastModified}`} label={`${f.name} • ${formatFileSize(f.size)}`} onDelete={() => removeFile(idx)} size="small" variant="outlined" />
            ))}
          </Stack>
