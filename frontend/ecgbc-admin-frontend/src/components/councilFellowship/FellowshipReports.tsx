...
import AddFellowshipReport from "./AddFellowshipReport";
import EditFellowshipReport from "./EditFellowshipReport";

interface FellowshipReportsProps {
  fellowshipId: string;
  fellowshipCertificateIssuedDate: string;
}
const FellowshipReports: React.FC<FellowshipReportsProps> = ({ fellowshipId, fellowshipCertificateIssuedDate }) => {
  ...
  useEffect(() => {
    if (fellowshipId) {
      dispatch(fetchReports({ fellowship: fellowshipId }));
    }
  }, [fellowshipId, dispatch]);

  const notReportedYears = getUnReportedYears(reports, fellowshipCertificateIssuedDate);
  const handleDownload = (fileUrl: string): void => {
    const link = document.createElement("a");
    link.href = fileUrl;
    link.download = `fellowship-report`; // Optionally set the filename, e.g., 'downloaded-file.pdf'
  ...
  const handleFileChange = (
    ...
    if (file) {
      dispatch(
        updateFellowshipReport({ updatedReport: { report: file, reportId } })
      );
    }
  };
  const handleDeleteReport = (reportId:string) => {

      dispatch(deleteFellowshipReport(reportId));
      handleClose();
  ...
  return (
    <>
        ...
        {loading ? (
          ...
          <>
            {notReportedYears.length > 0 && (
              <Box sx={{ mb: 2 }}>
                {/* You might want to create a FellowshipReportWarning component similar to ReportWarning */}
                    ...
                    >
                      ...
                      <Stack direction={'row'} alignItems={'center'} justifyContent={'flex-end'} sx={{ minWidth: 40 }}>
                        ...
                        {selectedReport && selectedReport.id === report.id && (
                          ...
                          >
                            {/* TODO: Uncomment when EditFellowshipReport component is created */}
                            {userHasPermission(staff?.role?.permissions??[],[Permissions.MEMBER_CHANGE,Permissions.REPORT_CHANGE]) && <MenuItem onClick={() => handleOpenEditModal(report)} disabled={updateReportloading}>
      ...
      {/* TODO: Create EditFellowshipReport component */}
      {selectedReport && (
        <EditFellowshipReport
          open={openEditModal}
...
export default FellowshipReports;
