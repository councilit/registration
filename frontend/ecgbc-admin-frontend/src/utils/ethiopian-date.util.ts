function getEthiopianDate(
  ...
  const offsetYears = 7; // Ethiopian year is 7 years behind Gregorian
  const newYearMonth = 9; // Ethiopian New Year starts in September
  const newYearDay = 12; // Ethiopian New Year day
  ...
  // Current Ethiopian New Year
  let currentEthiopianNewYear = new Date(
    Date.UTC(gregorianYear, newYearMonth - 1, newYearDay)
  );

  // Start with Ethiopian year
  let ethiopianYear = gregorianYear - offsetYears;

  let lastEthiopianNewYear;
  if (gregorianDate < currentEthiopianNewYear) {
    ethiopianYear -= 1;
    lastEthiopianNewYear = new Date(
      Date.UTC(gregorianYear - 1, newYearMonth - 1, newYearDay)
    );
  } else {
    lastEthiopianNewYear = currentEthiopianNewYear;
  }

  // Calculate difference in days
  const differenceInDays = Math.floor(
    (Number(gregorianDate) - Number(lastEthiopianNewYear)) / (1000 * 60 * 60 * 24)
  );

  // Calculate Ethiopian month and day
  let ethiopianMonth = Math.floor(differenceInDays / 30) + 1;
  let ethiopianDay = (differenceInDays % 30) + 2;
...
function convertEthiopianToGregorian(
  ...
  if (
    ethiopianMonth < newYearMonth ||
    (ethiopianMonth === newYearMonth && ethiopianDay < 11)
  ) {
  ...
  let gregorianNewYearDate = new Date(
    Date.UTC(gregorianYear, newYearMonth - 1, newYearDay)
  );
...
function toGeezNumber(number: number) {
  const geezDigits = ["፩", "፪", "፫", "፬", "፭", "፮", "፯", "፰", "፱"];
  const geezTens = ["፲", "፳", "፴", "፵", "፶", "፷", "፸", "፹", "፺"];
  const geezHundreds = ["፻"];
  const geezThousands = ["፲፻", "፪፻", "፫፻", "፬፻", "፭፻", "፮፻", "፯፻", "፰፻", "፱፻"];
  ...
  if (number >= 1000) {
    ...
    result +=
      (thousands === 1 ? "" : geezDigits[thousands - 1]) + geezThousands[0];
    number %= 1000;
  }
  ...
  if (number >= 100) {
    ...
    result +=
      (hundreds === 1 ? "" : geezDigits[hundreds - 1]) + geezHundreds[0];
    number %= 100;
  }
...
function toEthiopianDateText(date: string) {
  ...
  return toEthiopianGeezLong(
    gregorianDate.getFullYear(),
    gregorianDate.getMonth() + 1,
    gregorianDate.getDate()
  );
}
function toEthiopianGeezLong(
  gregorianYear: any,
  gregorianMonth: any,
  gregorianDay: any
) {
  ...
  let { ethiopianYear, ethiopianMonth, ethiopianDay } = getEthiopianDate(
    gregorianYear,
    gregorianMonth,
    gregorianDay
  );
  console.log(ethiopianDay, ethiopianYear, ethiopianMonth);
...
function toEthiopianShortDate(ethYear: any, ethMonth: any, ethnDay: any) {
  const { ethiopianYear, ethiopianMonth, ethiopianDay } = getEthiopianDate(
    ethYear,
    ethMonth,
    ethnDay
  );
  return {
    ...
    day: ethiopianDay,
    text: `${ethiopianDay}/${ethiopianMonth}/${ethiopianYear}`,
  };
}
...
const ethiopianCalendar = {
  ...
  getYearsInGeez: () => {
    ...
    const years = Array.from(
      { length: range },
      (_, i) => currentYear - offsetYears - i
    );
    return years.map((year) => toGeezNumber(year));
  },
};

function getNextEthiopianHoliday(today: Date) {
  ...
  const ethiopianHolidays = [
    {
      ...
      amharicName: "የዓድዋ ድል በዓል",
      link: "https://en.wikipedia.org/wiki/Adwa_Victory_Day",
      notes:
        "Commemorates Ethiopians victory over Italy at Battle of Adwa in 1896.",
    },
    {
      date: "01-May",
      englishName: "International Workers Day",
      amharicName: "ዓለም አቀፍ የሠራተኞች ቀን",
      link: "",
      notes: "",
    },
    {
      ...
      amharicName: "የአርበኞች ቀን",
      notes:
        "Commemorates the 1941 entering of Emperor Haile Selassie into Addis Ababa amidst Second World War.",
    },
    {
      date: "28-May",
      englishName: "Downfall of the Derg",
      amharicName: "ደርግ የወደቀበት ቀን",
      link: "https://en.wikipedia.org/wiki/Fall_of_the_Derg_regime",
      notes: "Commemorates the end of the Derg junta in 1991.",
    },
    {
      date: "11-Sep",
      ...
      notes: "New Year of Ethiopia and Eritrea",
    },
    {
      date: "26-Oct",
      englishName: "Defense Day",
      amharicName: "የመከላከያ ቀን",
      link: "https://en.wikipedia.org/wiki/Defense_Day_(Ethiopia)",
      notes:
        "Commemorating the formation of the Ethiopian National Defense Force in 1907",
    },
    {
      date: "07-Jan",
      englishName: "Ethiopian Christmas",
      amharicName: "ገና",
      notes: "",
    },
    {
      ...
      amharicName: "ብርሐነ ጥምቀት",
      notes:
        "Celebrated on January 19th, marks the Ethiopian Orthodox Epiphany.",
    },
    {
      date: "27-Sep",
      englishName: "Meskel",
    ...
    },
    {
      ...
      englishName: "Easter",
      amharicName: "ፋሲካ",
      notes: "Commemorating the resurrection of Jesus Christ.",
    },
  ];
  ...
  const holidaysWithDateObjects = ethiopianHolidays.map((holiday) => {
    ...
    const dateThisYear = new Date(today.getFullYear(), monthIndex, Number(day));
...
function fromGeezNumber(geezNumber: string) {
  const geezToDigit: any = {
    "፩": 1,
    "፪": 2,
    "፫": 3,
    "፬": 4,
  ...
  geezNumber.split("").forEach((char: string) => {
